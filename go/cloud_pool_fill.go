...
// 后台灌池像两口水井共养一池鱼：用 probe_accounts 定期向启用源打票，
// 满血 __cflb/__oailb 都放入响应侧 harvest 使用的同一个全局路由池，业务池模式拿来引导。
// FC 是公网阿里云函数，Relay 是本机住宅代理；开关、地址、前置代理各自独立，可并行。
// 账号 access_token 通过只读管理 API 取得，复用 probeClient/probeDownloadCreds，
// 不写账号文件、不刷新 OAuth；配置变化或关闭时干净取消，不让旧水泵在后台装忙。
var poolFiller = struct {
	sync.Mutex
	cancel context.CancelFunc
}{}

// fillSourceStat 记单个灌池源上一轮成绩，谁打来水谁记账，不混成一锅。
...
// poolFillStatus 是灌池给面板递的小黑板：运行状态、节奏、范围、每个源上一轮成绩。
// 配置变更、关闭和每轮结束时擦写，别让昨日营业牌冒充今日开张。
...
// 新配置先请旧循环收摊；任一具名源或旧 pool_fill 开着，才让新班水泵上岗。
...
// fillTargetsSummary 给日志报源名和脱敏端点，代理凭据不许举着喇叭喊。
...
// 一轮只取一次账号 token，再让各启用源逐账号、逐模型打票。
// 成功的 pair 都归全局池这一本账，不另藏小金库。
...
// cloudPoolFillSource 让一个灌池源跑完本轮，再把成绩记在它自己名下，功劳不能串桌。
