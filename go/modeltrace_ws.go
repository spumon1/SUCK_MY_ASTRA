...
// Test B 的 WS 客户端:本机用 WS 连 CPA 业务端点(不是上游),发 codex 挑战帧,读
// output_text。走的是真实客户端 WS 路径 —— CPA 收到后按正常业务处理(插件注入池票、
...
func clientTurnWS(ctx context.Context, endpoint, apiKey, accountID, model, prompt string) (served, output string, err error) {
	...
	// 发 codex 业务挑战帧(response.create),不带票 —— 票由 CPA 注入链路负责。
...
// wsCollectOutput 累积 response.output_text.delta,读到终态返回。
