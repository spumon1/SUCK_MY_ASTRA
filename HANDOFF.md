...
1. **paired-312 → TryNext**(`probe_runner.go` `probeConsume`):200 响应带
   ...
   出口；日志含 `degraded` 与 `(pair still pooled: …)`。这家不记成功，换个出口继续敲门。
...
3. **测试自死锁修复**:`TestProbeMintsBareAndPools` 的 `defer Unlock` 曾
   持锁二次 harvest → 改显式 Unlock，套件才得以跑完。锁是自己拿的，门也是自己堵的，
   这次先放开手再办事。
...
6. **观测分类放宽（实测驱动）**:`markRouteCookieOutcomeLocked` 对
   ...
   归因门从「292/312 白名单」放宽为 `len>0` —— 长度不再被旧白名单一眼赶出门。锁定测试：
...
8. **TTL 中位数=3900，expireAt 改取凭证自身死线**：7 次连采 `__oailb`
   ...
   声明 3600，且 pair 实测活过它——声明先分清来自哪张纸,不能只听门口吆喝）。
   `parseSetCookieLine` 现对 JWT 值优先取内嵌 `exp`（`jwtExpiresAt`，
   不验签——读的是上游自己的声明），属性作兜底。`ttl_seconds` 默认改
   **3900**：窗口 = `min(seenAt+ttl, expireAt)` —— expireAt 硬 cap 从
   「保守属性」变成「凭证真死线」，原预警随之消解。测试：
...
1. ~~**分类器按计划/账号分档**~~ **已落地（按桶学习）**：上游统一
   ...
   585B/33 块），任何固定映射表都会随格式升级失效——票版换了新戏服，旧尺寸表不能永远当裁判。
   `bucketObservation` 新增 `signed_lens` 计数表：未识别长度首现记 `other`，桶内复发
   ...
   **降级签名已实测定位**：不在长度上，而在 SSE 载荷 —— 响应头
   ...
   降级事件是 `response.model` 被换成 fallback——换了人上场才算降级,
   告示牌站上舞台也不算换了演员。插件已落地检测：header-init 时按 RequestID 武装
   `pendingModelScan`，首个载荷块取 `"model"` 字段，与请求模型不符即记
   `limited` + pair 记 bad（同 312 的权重）;feed 事件新增 `served` 字
   段，看板判定语改「降级 → X」。健康账号实测阴性：约 50 发覆盖 7 模型
   × 6 节点（125/95/88/157/97/179 转向）× 变载荷 × 工具 × effort ×
   突发 × 安全评审 prompt，全一致；请求侧全部旋钮（buffering 头/
   service_tier/store/meta/routing-hint）均不可关降级。续轮实测：
   `previous_response_id` 被拒（400)，续跑只靠回放 turn-state 头，
   被接受时 ts=0 不再铸票（=steered 签名）。
2. **README / DEPLOY 大面积旧机制叙述**：仍写 `inject_mode`/
   `harvest_inband`/per-bucket store/Fernet 票替换——旧剧本仍在讲上一场戏,
   整篇重写。
3. **ui.html 旧文案**：死字段读取已修——矩阵格子的倒计时改读
   ...
   DEPLOY 重写同属一项，等明确措辞再动——牌匾还唱旧戏,待措辞定稿再换,不假装这项已经收工。
...
reminted)。采样至 age=2254s 全存活即停，死亡年龄未测——样本一直没退场,
先存档,不能替它编一个谢幕时刻。

**替代实测已落地**:不替没测完的寿命编结局,另取 7 次连采 `__oailb` JWT `exp-iat` 中位数 **3900**
...
- 转向验证：跨出口携带 pair 重放 → 200 + 332,**边缘不重铸 pair**
  （只回 `__cf_bm`)= 「steered 请求的典型响应」实证——房卡已经递上,
  柜台这次没有另开一张。
...
- **上游已统一 X-Codex-Turn-State 格式**：同一 prolite 凭据 × 7 模型
  ...
  响应铸票、同 turn 续跑只回放首值——台词换了长度,新 turn 与续跑的出场顺序没改。
...
# 看 x-codex-turn-state 长度与 set-cookie 名即可（值勿打印——天机不可泄露）。
```
